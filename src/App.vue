  ...
  methods: {
    ...
    handleNodeMove(moveData) {
      this.nodes.forEach(node => {
        if(node.id == moveData.node.id) {
          node.positionX = this.node_start_move_info.positionX +  moveData.dx
          node.positionY = this.node_start_move_info.positionY +  moveData.dy
        }
      })
    },
    handleNodeMoveEnd(event) {
      console.log('节点移动结束')
    },
