<template>
  <g :id="'_' + node.id" :transform="'translate('+node.positionX+','+node.positionY+')'">
    ...
    <in-port v-for="(item,index) in node.inPorts" :pid="item.id" :type="item.type" :portNum="node.inPorts.length" v-on:on-add-path='addPath' :isConnected = "item.isConnected" :key="'in'+ '_' + index"></in-port>
    <out-port v-for="(item, index) in node.outPorts" :pid="item.id" :type="item.type" :portNum="node.outPorts.length" :key="'out'+ '_' + index"></out-port>
  </g>
</template>
